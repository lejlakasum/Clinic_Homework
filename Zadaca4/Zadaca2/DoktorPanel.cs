    ...
    public partial class DoktorPanel : Form
    {
        ...
        private void comboBox_PretragaPo_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                comboBox_OdabirPac_Ord.Items.Clear();
                Parallel.ForEach(Glavna.IndeksiPacijenataiNaKlinici, indeks =>
              {
                  this.Invoke(new Action(() => comboBox_OdabirPac_Ord.Items.Add(Glavna.klinika17280_1.RegistrovaneOsobe[indeks].Ime + " " +
        ...
        private void comboBox_OdaberiPacijenta_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Parallel.ForEach(pac.KartonPacijenta.ListaPregleda, p =>
           {
               this.Invoke(new Action(() => listBox_KartonPregled.Items.Add(p.PregledKojiSeObavlja.ImePregleda + " " + p.DatumPregleda.ToShortDateString())));
           });
            

            //Popunjavanje liste pregleda
            comboBox_OdabirPregleda.Items.Clear();
            Parallel.ForEach(pac.SpisakPregleda, p =>
          {
              if (p.Value == true && p.Key.OrdinacijaUKojojSeObavlja.ImeOrdinacije == trenutnaOrdinacija.ImeOrdinacije)
              {
                  this.Invoke(new Action(() => comboBox_OdabirPregleda.Items.Add(p.Key.ImePregleda)));
              }
          });
        }
